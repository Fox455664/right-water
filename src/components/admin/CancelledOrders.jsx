...
const CancelledOrders = () => {
  ...
  useEffect(() => {
    const fetchCancelledOrders = async () => {
      ...
      try {
        ...
        const cancelledOrders = orderSnapshot.docs
          .map(doc => {
            const data = doc.data();
        ...
        setError("حدث خطأ أثناء تحميل الطلبات الملغية.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-10">
        ...
        <p className="ml-4 text-lg text-muted-foreground">جاري تحميل الطلبات الملغية...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-primary text-right">الطلبات الملغية</h2>
      {orders.length === 0 ? (
        <p className="text-muted-foreground text-center py-8">لا توجد طلبات ملغية حالياً.</p>
      ) : (
