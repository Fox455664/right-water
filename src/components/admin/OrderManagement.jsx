...
chevron_right
ولا يهمك يا صديقي، لا حمار ولا حاجة خالص. كلنا بنتلخبط في أسماء الحقول دي بالذات لما بنشتغل بين الواجهة وقاعدة البيانات. الموضوع بسيط جداً.
أتفضل، ده الكود الكامل لملف OrderManagement.jsx بعد ما صلحت فيه كل الأماكن اللي كانت بتقرأ البيانات غلط.
كل اللي عليك تعمله هو إنك تحدد كل الكود في ملف OrderManagement.jsx اللي عندك وتمسحه، وتحط الكود ده مكانه.
import React, { useState, useEffect, useMemo, useCallback, useRef } from 'react';
import { motion } from 'framer-motion';
import { Button } from '@/components/ui/button';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
                  ...
                  return (
                    <TableRow key={order.id} data-state={selectedOrders.includes(order.id) ? "selected" : ""} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      ...
                      <TableCell className="px-3 py-4">
                        ...
                        <div className="text-xs text-slate-500 dark:text-slate-400">{order.customerInfo?.email}</div>
                      </TableCell>
                      <TableCell className="px-3 py-4">{formatDate(order.createdAt)}</TableCell>
                      <TableCell className="px-3 py-4">{formatPrice(order.totalAmount)}</TableCell>
                      <TableCell className="px-3 py-4">
                        <Select value={order.status} onValueChange={(newStatus) => handleStatusChange(order.id, newStatus)}>
                          <SelectTrigger className={`w-full text-xs h-8 px-2 py-1 rounded-md border-0 focus:ring-0 focus:ring-offset-0 ${statusInfo.color} text-white`}>
                            <SelectValue />
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p><strong>الاسم:</strong> {currentOrderDetails?.customerInfo?.name}</p>
                  <p><strong>البريد:</strong> {currentOrderDetails?.customerInfo?.email}</p>
                  <p><strong>الهاتف:</strong> {currentOrderDetails?.customerInfo?.phone}</p>
                </div>
                <div>
                  <h3 className="font-semibold text-lg mb-2 text-slate-700 dark:text-slate-200">عنوان الشحن</h3>
                  <p>{currentOrderDetails?.customerInfo?.address}</p>
                  <p>{currentOrderDetails?.customerInfo?.city}</p>
                </div>
              </div>
              <div>
                      ...
                      <TableRow key={item.id}>
                        <TableCell>
                          <div className="flex items-center">
                            <img src={item.image || 'https://via.placeholder.com/50'} alt={item.name} className="w-10 h-10 object-cover rounded-md ml-3 rtl:mr-3" />
                            {item.name}
                          </div>
                        </TableCell>
