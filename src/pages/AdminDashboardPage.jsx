...
// ... (مكونات StatCard و ManagementLinkCard بدون تغيير)
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="h-full">
    <Link to={to} className="block h-full group">
      <Card className={`${bgColorClass} text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 h-full flex flex-col justify-between p-6 rounded-lg`}>
        ...
        <div className="mt-4 text-right"><span className="text-sm font-medium group-hover:underline">الانتقال →</span></div>
      </Card>
    </Link>
  </motion.div>
);
...
const AdminDashboardPage = () => {
  ...
  const quickStatsCards = [
    { title: 'إجمالي المستخدمين', value: loading ? '...' : staticStats.users, icon: <Users className="h-6 w-6 text-sky-500" />, color: 'sky', linkTo: '/admin/users' },
    { title: 'إجمالي الطلبات', value: loading ? '...' : dynamicStats.totalOrders, icon: <ListOrdered className="h-6 w-6 text-green-500" />, color: 'green', linkTo: '/admin/orders' },
    ...
    { title: 'المنتجات النشطة', value: loading ? '...' : staticStats.products, icon: <Package className="h-6 w-6 text-purple-500" />, color: 'purple', linkTo: '/admin/products' },
  ];

  const managementLinks = [
    { to: '/admin/products', title: 'إدارة المنتجات', description: 'إضافة، تعديل، وحذف المنتجات.', icon: <Package className="h-8 w-8 text-white" />, bgColorClass: 'bg-gradient-to-br from-sky-500 to-sky-700' },
    { to: '/admin/orders', title: 'إدارة الطلبات', description: 'متابعة ومعالجة طلبات العملاء.', icon: <ListOrdered className="h-8 w-8 text-white" />, bgColorClass: 'bg-gradient-to-br from-green-500 to-green-700' },
    { to: '/admin/users', title: 'إدارة المستخدمين', description: 'عرض وتعديل معلومات المستخدمين.', icon: <Users className="h-8 w-8 text-white" />, bgColorClass: 'bg-gradient-to-br from-purple-500 to-purple-700' },
    { to: '/admin/settings', title: 'إعدادات الموقع', description: 'تكوين إعدادات المتجر العامة.', icon: <Settings className="h-8 w-8 text-white" />, bgColorClass: 'bg-gradient-to-br from-slate-500 to-slate-700' },
  ];
    ...
    <div className="bg-slate-100 dark:bg-slate-900 p-4 sm:p-6 lg:p-8">
      ...
      >
        ...
        <div className="flex items-center gap-2 mt-4 sm:mt-0">
          ...
          <Button variant="outline"><MessageSquare className="mr-2 rtl:ml-2 rtl:mr-0 h-5 w-5" /> رسائل الدعم</Button>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div className="lg:col-span-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
          <Card className="bg-white dark:bg-slate-800 shadow-lg h-full">
            <CardHeader><CardTitle className="text-xl font-semibold text-slate-700 dark:text-slate-200">أحدث الطلبات</CardTitle></CardHeader>
            <CardContent>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
          <Card className="bg-white dark:bg-slate-800 shadow-lg h-full">
            <CardHeader><CardTitle className="text-xl font-semibold text-slate-700 dark:text-slate-200">نظرة عامة على النشاط</CardTitle></CardHeader>
            <CardContent>
              <div className="h-64 flex items-center justify-center text-slate-400 dark:text-slate-500">
                <BarChart3 size={48} /><p className="ml-4 rtl:mr-4">مخطط النشاط سيكون هنا</p>
              </div>
            </CardContent>
          </Card>
        </motion.div>
